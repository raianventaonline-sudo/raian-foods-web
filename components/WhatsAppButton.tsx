...
const buildMessage = (pathname: string, isFromQr: boolean) => {
  const cleanPath = pathname.replace(/^\/en/, "");
  const productMatch = cleanPath.match(/^\/productos\/([^/]+)/);
  const recipeMatch = cleanPath.match(/^\/recetas\/([^/]+)/);
  const url = `${siteConfig.siteUrl}${pathname}`;

  if (productMatch) {
    const label = slugToLabel(productMatch[1]);
    return isFromQr
      ? `Hola, he comprado el producto "${label}" (he escaneado el QR del envase) y quiero hacer una consulta.\n${url}`
      : `Hola, tengo una consulta sobre el producto "${label}".\n${url}`;
  }

  if (recipeMatch) {
    const label = slugToLabel(recipeMatch[1]);
    return isFromQr
      ? `Hola, he comprado un producto RAIAN y estaba viendo la receta "${label}" (he escaneado el QR del envase). Quiero hacer una consulta.\n${url}`
      : `Hola, tengo una consulta sobre la receta "${label}".\n${url}`;
  }

  return isFromQr
    ? `Hola, he escaneado el QR de un producto RAIAN y quiero hacer una consulta.\n${url}`
    : `Hola, tengo una consulta sobre un producto de RAIAN Foods.\n${url}`;
};

function WhatsAppButtonInner() {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const isFromQr = searchParams.get("ref") === "qr";
  const message = buildMessage(pathname, isFromQr);
  ...
  return (
    <a
      ...
      className="raian-pulse-green fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lg transition hover:scale-110 hover:shadow-xl"
    >
